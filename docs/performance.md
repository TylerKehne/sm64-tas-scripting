...
- On MSVC 19.51 `LevelStack::operator[]` is not inlined into `Script::GetInputsMetadata`
  ...
  the copy). Found 2026-09-15 with PMC counters (performance-changelog.md); ROADMAP 3.19.
...
- Tight loops are sensitive to code layout. Adding code elsewhere in the binary has moved
  ...
  code stayed put. On MSVC 19.51 the same rows move too, and by more than the gate: master
  with one unrelated benchmark appended read them 15 to 28% over master's own binary
  (performance-changelog.md, 2026-09-15; the rule for those rows is ROADMAP 3.18). A shift
  on a benchmark whose code did not change is layout: confirm with a re-run, say so in the
  change log, and re-save the baseline.
