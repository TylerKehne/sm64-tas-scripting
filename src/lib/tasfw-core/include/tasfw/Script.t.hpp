...
void Script<TResource>::AdvanceFrameWrite(Inputs inputs)
{
	...
	// Erase all saves, cached saves and inputs, tracked loads and frame
	// counters after this point, as well as the cached input on this frame
	inputsCache[_adhocLevel].erase(
		inputsCache[_adhocLevel].lower_bound(currentFrame),
		inputsCache[_adhocLevel].end());
	frameCounter[_adhocLevel].erase(
		frameCounter[_adhocLevel].upper_bound(currentFrame),
		frameCounter[_adhocLevel].end());
	saveBank[_adhocLevel].erase(
		saveBank[_adhocLevel].upper_bound(currentFrame),
		saveBank[_adhocLevel].end());
	saveCache[_adhocLevel].erase(
		saveCache[_adhocLevel].upper_bound(currentFrame),
		saveCache[_adhocLevel].end());
	loadTracker[_adhocLevel].erase(
		loadTracker[_adhocLevel].upper_bound(currentFrame),
		loadTracker[_adhocLevel].end());
...
template <derived_from_specialization_of<Resource> TResource>
void Script<TResource>::ApplyChildDiff(
	const BaseScriptStatus& status, int64_t initialFrame)
{
	...
	// Erase all saves, cached saves, and frame counters after this point
	inputsCache[_adhocLevel].erase(
		inputsCache[_adhocLevel].lower_bound(firstFrame),
		inputsCache[_adhocLevel].end());
	frameCounter[_adhocLevel].erase(
		frameCounter[_adhocLevel].upper_bound(firstFrame),
		frameCounter[_adhocLevel].end());
	saveBank[_adhocLevel].erase(
		saveBank[_adhocLevel].upper_bound(firstFrame),
		saveBank[_adhocLevel].end());
	saveCache[_adhocLevel].erase(
		saveCache[_adhocLevel].upper_bound(firstFrame),
		saveCache[_adhocLevel].end());
	loadTracker[_adhocLevel].erase(
		loadTracker[_adhocLevel].upper_bound(firstFrame),
		loadTracker[_adhocLevel].end());

	// Apply diff. State is already synced from child script, so no need to
	// update it
	int64_t frame = firstFrame;
	while (frame <= lastFrame)
	{
		if (status.m64Diff.frames.count(frame))
			BaseStatus[_adhocLevel].m64Diff.frames[frame] =
				status.m64Diff.frames.at(frame);
...
InputsMetadata<TResource> Script<TResource>::GetInputsMetadata(int64_t frame)
{
	if (!_parentScript)
		throw std::runtime_error(
			"Failed to get inputs because of missing parent script");
	...
	for (int64_t adhocLevel = _adhocLevel; adhocLevel >= 0; adhocLevel--)
	{
		if (!stateOwner)
		{
			if (!BaseStatus[adhocLevel].m64Diff.frames.empty() &&
				BaseStatus[adhocLevel].m64Diff.frames.begin()->first <= frame)
			{
				stateOwner			 = this;
		...
		if (BaseStatus[adhocLevel].m64Diff.frames.contains(frame))
			return InputsMetadata<TResource>(
				BaseStatus[adhocLevel].m64Diff.frames[frame], frame, stateOwner,
				stateOwnerAdhocLevel);
	...
	throw std::runtime_error(
		"Failed to get inputs, possible error in recursion logic.");
	return InputsMetadata<TResource>();
}

template <derived_from_specialization_of<Resource> TResource>
InputsMetadata<TResource> TopLevelScript<TResource>::GetInputsMetadata(
	int64_t frame)
{
	// State owner determines what frame counter needs to be incremented
	int64_t stateOwnerAdhocLevel = -1;
	...
	for (int64_t adhocLevel = this->_adhocLevel; adhocLevel >= 0; adhocLevel--)
	{
		if (stateOwnerAdhocLevel == -1)
		{
			if (!this->BaseStatus[adhocLevel].m64Diff.frames.empty() &&
				this->BaseStatus[adhocLevel].m64Diff.frames.begin()->first <=
					frame)
				stateOwnerAdhocLevel = adhocLevel;
		}
		...
		if (this->BaseStatus[adhocLevel].m64Diff.frames.contains(frame))
			return InputsMetadata<TResource>(
				this->BaseStatus[adhocLevel].m64Diff.frames[frame], frame, this,
				stateOwnerAdhocLevel);

		if (this->inputsCache[adhocLevel].contains(frame))
		{
			InputsMetadata<TResource> metadata =
				this->inputsCache[adhocLevel][frame];
			if (stateOwnerAdhocLevel == -1)
	...
	if (_m64->frames.count(frame))
		return InputsMetadata<TResource>(
			_m64->frames[frame], frame, this, stateOwnerAdhocLevel,
			InputsMetadata<TResource>::InputsSource::ORIGINAL);

	// Default to no input
	// For the purposes of the frame counter, mark as adhoc level 0.
	return InputsMetadata<TResource>(
		Inputs(0, 0, 0), frame, this, stateOwnerAdhocLevel,
		InputsMetadata<TResource>::InputsSource::DEFAULT);
}

template <derived_from_specialization_of<Resource> TResource>
InputsMetadata<TResource> Script<TResource>::GetInputsMetadataAndCache(
	int64_t frame)
{
...
template <derived_from_specialization_of<Resource> TResource>
uint64_t Script<TResource>::GetFrameCounter(
	InputsMetadata<TResource> cachedInputs)
{
	if (!cachedInputs.stateOwner
			 ->frameCounter[cachedInputs.stateOwnerAdhocLevel]
			 .contains(cachedInputs.frame))
		cachedInputs.stateOwner->frameCounter[cachedInputs.stateOwnerAdhocLevel]
											 [cachedInputs.frame] = 0;

	return cachedInputs.stateOwner
		->frameCounter[cachedInputs.stateOwnerAdhocLevel][cachedInputs.frame];
}

template <derived_from_specialization_of<Resource> TResource>
uint64_t Script<TResource>::IncrementFrameCounter(
	InputsMetadata<TResource> cachedInputs)
{
	if (!cachedInputs.stateOwner
			 ->frameCounter[cachedInputs.stateOwnerAdhocLevel]
...
SaveMetadata<TResource> Script<TResource>::GetLatestSave(int64_t frame)
{
	if (resource->initialFrame > frame)
		throw std::runtime_error(
			"Error: attempted to load frame prior to initial frame");
	...
	for (int64_t adhocLevel = _adhocLevel; adhocLevel >= 0; adhocLevel--)
	{
		...
		auto save = saveBank[adhocLevel].empty() ||
				earlyFrame < saveBank[adhocLevel].begin()->first ?
		...
		if (save != saveBank[adhocLevel].end())
		{
			if (!save->second.isValid())
				...
				bestSave =
					SaveMetadata<TResource>(this, save->first, adhocLevel);
		}

		// Check for cached save
		auto cachedSave = saveCache[adhocLevel].empty() ||
				earlyFrame < saveCache[adhocLevel].begin()->first ?
			saveCache[adhocLevel].end() :
			std::prev(saveCache[adhocLevel].upper_bound(earlyFrame));

		if (cachedSave != saveCache[adhocLevel].end())
		{
			// This is the purpose of caching saves: end recursion when a cached
			// save is found. Boosts performance.
			if (cachedSave->second.IsValid())
			{
				if (cachedSave->first >= bestSave.frame)
				{
					// However, if there was a load between the target frame and
					// the cached save, it may not be optimal and we should
					// continue recursion
					auto loadAfterCachedSave =
						loadTracker[_adhocLevel].lower_bound(cachedSave->first);
					if (loadAfterCachedSave == loadTracker[_adhocLevel].end() ||
						*loadAfterCachedSave < frame)
				...
				saveCache[adhocLevel].erase(
					cachedSave->first);	 // Delete stale cached save
		}

		// Don't search past start of m64 diff to avoid desync
		earlyFrame = !BaseStatus[adhocLevel].m64Diff.frames.empty() ?
			(std::min)(
				BaseStatus[adhocLevel].m64Diff.frames.begin()->first,
				(uint64_t) earlyFrame) :
			frame;

		// If save is not before the start of the diff, we have the best
		// possible save, so return it
...
SaveMetadata<TResource> Script<TResource>::GetLatestSaveAndCache(int64_t frame)
{
	...
	saveCache[_adhocLevel][save.frame] =
		save;  // Cache save to save recursion time later

	// Tracked loads between this frame and the cached save frame are obsoloted
	// by this one
...
void Script<TResource>::Load(uint64_t frame)
{
	...
	// parent. If target frame is in future, check if faster to frame advance or
	// load.
	...
	if (frame < currentFrame)
	{
		...
		latestSave.frame > currentFrame &&
		resource->shouldLoad(latestSave.frame - currentFrame))
		resource->LoadState(latestSave.GetSlotHandle()->slotId);
	...
	while (currentFrame++ < frame)
	{
		...
		// Estimate future frame advances from aggregate of historical frame
		// advances on this input segment If it reaches a certain threshold,
		// creating a save is performant
		if (resource->shouldSave(frameCounter))
		{
			SaveMetadata<TResource> cachedSave = cachedInputs.stateOwner->Save(
				cachedInputs.stateOwnerAdhocLevel);
...
// Load method specifically for Script.Execute() and Script.Modify(), checks for
// desyncs
template <derived_from_specialization_of<Resource> TResource>
...
void Script<TResource>::Rollback(uint64_t frame)
{
	...
	{
		int64_t firstFrame =
			BaseStatus[_adhocLevel].m64Diff.frames.lower_bound(frame)->first;
...
// Same as Rollback, but starts from current frame. Useful for scripts that edit
// past frames
template <derived_from_specialization_of<Resource> TResource>
void Script<TResource>::RollForward(int64_t frame)
{
	// Roll back diff and savebank to current frame. Note that roll forward on
	// diff includes current frame.
	uint64_t currentFrame = GetCurrentFrame();

	if (!BaseStatus[_adhocLevel].m64Diff.frames.empty())
	{
		int64_t firstFrame =
			BaseStatus[_adhocLevel].m64Diff.frames.begin()->first;

		auto inputsUpperBound =
			BaseStatus[_adhocLevel].m64Diff.frames.upper_bound(frame);
		if (inputsUpperBound ==
				BaseStatus[_adhocLevel].m64Diff.frames.begin() &&
			BaseStatus[_adhocLevel].m64Diff.frames.size() == 1)
			inputsUpperBound = BaseStatus[_adhocLevel].m64Diff.frames.end();
		else
			inputsUpperBound = std::prev(inputsUpperBound);

		BaseStatus[_adhocLevel].m64Diff.frames.erase(
			BaseStatus[_adhocLevel].m64Diff.frames.begin(), inputsUpperBound);
...
// Internal version of Save() that specifies adhoc level, that can be called by
// a child script
template <derived_from_specialization_of<Resource> TResource>
SaveMetadata<TResource> Script<TResource>::Save(int64_t adhocLevel)
{
	// Desyncs should always clear future saves, so if a save already exists
	// there is no need to overwrite it
	int64_t currentFrame = GetCurrentFrame();
	if (!saveBank[adhocLevel].contains(currentFrame))
	{
		saveBank[adhocLevel].emplace(
			std::piecewise_construct, std::forward_as_tuple(currentFrame),
...
void Script<TResource>::OptionalSave()
{
	...
	for (int64_t frame = latestSaveFrame; frame <= currentFrame; frame++)
	{
		...
		// Increment AFTER save check. We want to know if a save on the NEXT
		// frame is worthwhile based on the counter from this frame
...
template <derived_from_specialization_of<Resource> TResource>
AdhocBaseScriptStatus Script<TResource>::ExecuteAdhoc(
	AdhocScript auto adhocScript)
{
...
AdhocScriptStatus<TAdhocCustomScriptStatus> Script<TResource>::ExecuteAdhoc(
	F adhocScript)
{
	int64_t initialFrame = GetCurrentFrame();
	...
	BaseScriptStatus baseStatus =
		ExecuteAdhocBase([&]() { return adhocScript(customStatus); });
	Revert(initialFrame, baseStatus.m64Diff);

	return AdhocScriptStatus<TAdhocCustomScriptStatus>(
		baseStatus, customStatus);
}

template <derived_from_specialization_of<Resource> TResource>
AdhocBaseScriptStatus Script<TResource>::ModifyAdhoc(
	AdhocScript auto adhocScript)
{
	int64_t initialFrame = GetCurrentFrame();
...
template <
	class TAdhocCustomScriptStatus,
	AdhocCustomStatusScript<TAdhocCustomScriptStatus> F>
AdhocScriptStatus<TAdhocCustomScriptStatus> Script<TResource>::ModifyAdhoc(
	F adhocScript)
{
	int64_t initialFrame = GetCurrentFrame();
...
template <
	class TAdhocCustomScriptStatus,
	AdhocCustomStatusScript<TAdhocCustomScriptStatus> F>
AdhocScriptStatus<TAdhocCustomScriptStatus> Script<TResource>::TestAdhoc(
	F&& adhocScript)
{
	auto status =
		ExecuteAdhoc<TAdhocCustomScriptStatus>(std::forward<F>(adhocScript));
	status.m64Diff = M64Diff();

	return status;
}

// Root method for Compare() template recursion. This will be called when there
// are no scripts left to compare the incumbent to.
template <derived_from_specialization_of<Resource> TResource>
template <class TCompareStatus>
	requires(std::derived_from<TCompareStatus, CompareStatus<TCompareStatus>>)
AdhocScriptStatus<TCompareStatus> Script<TResource>::Compare(
	const AdhocScriptStatus<TCompareStatus>& status1)
{
	return status1;
}

// Main recursive method for comparing ad hoc scripts. Only the root and leaf
	...
	G&&... adhocScripts)
{
	...
	{
		if (!status2.executed)
			return Compare<TCompareStatus>(
				status1, std::forward<G>(adhocScripts)...);

		return Compare<TCompareStatus>(
			status2, std::forward<G>(adhocScripts)...);
	}

	return Compare<TCompareStatus>(
		status1.Comparator(status1, status2), std::forward<G>(adhocScripts)...);
}
	...
	F&& adhocScript1, G&& adhocScript2, H&&... adhocScripts)
{
	...
	if (!status1.executed)
	{
		if (!status2.executed)
			return Compare<TCompareStatus>(
				status1, std::forward<H>(adhocScripts)...);

		return Compare<TCompareStatus>(
			status2, std::forward<H>(adhocScripts)...);
	}

	return Compare<TCompareStatus>(
		status1.Comparator(status1, status2), std::forward<H>(adhocScripts)...);
}

// Root method for ModifyCompare() template recursion. This will be called when
// there are no scripts left to compare the incumbent to.
template <derived_from_specialization_of<Resource> TResource>
template <class TCompareStatus>
	requires(std::derived_from<TCompareStatus, CompareStatus<TCompareStatus>>)
AdhocScriptStatus<TCompareStatus> Script<TResource>::ModifyCompare(
	...
	F&& adhocScript2, G&&... adhocScripts)
{
	...
	if (status2.executed && status2.Terminator(status2))
	{
		// Revert state if execution failed or diff is empty, otherwise apply
		// diff
	...
	{
		if (!status2.executed)
			return ModifyCompare<TCompareStatus>(
				initialFrame, status1, std::forward<G>(adhocScripts)...);

		return ModifyCompare<TCompareStatus>(
			initialFrame, status2, std::forward<G>(adhocScripts)...);
	}

	return ModifyCompare<TCompareStatus>(
		initialFrame, status1.Comparator(status1, status2),
		std::forward<G>(adhocScripts)...);
}

// Leaf method for comparing ad-hoc scripts and applying the result. This is the
// one the user will call.
template <derived_from_specialization_of<Resource> TResource>
template <
	class TCompareStatus, AdhocCustomStatusScript<TCompareStatus> F,
	AdhocCustomStatusScript<TCompareStatus> G,
	...
	F&& adhocScript1, G&& adhocScript2, H&&... adhocScripts)
{
	int64_t initialFrame = GetCurrentFrame();
	auto status1 =
		ExecuteAdhocNoRevert<TCompareStatus>(std::forward<F>(adhocScript1));
	if (status1.executed && status1.Terminator(status1))
	{
	...
	Revert(initialFrame, status1.m64Diff);
	auto status2 =
		ExecuteAdhocNoRevert<TCompareStatus>(std::forward<G>(adhocScript2));
	if (status2.executed && status2.Terminator(status2))
	{
		// Revert state if diff is empty, otherwise apply diff
	...
	if (!status1.executed)
	{
		if (!status2.executed)
			return ModifyCompare<TCompareStatus>(
				initialFrame, status1, std::forward<H>(adhocScripts)...);

		return ModifyCompare<TCompareStatus>(
			initialFrame, status2, std::forward<H>(adhocScripts)...);
	}

	return ModifyCompare<TCompareStatus>(
		initialFrame, status1.Comparator(status1, status2),
		std::forward<H>(adhocScripts)...);
}
...
template <
	...
	AdhocCustomStatusScript<TCompareStatus>... H>
	requires(std::derived_from<TCompareStatus, CompareStatus<TCompareStatus>>)
AdhocScriptStatus<TCompareStatus> Script<TResource>::TestCompare(
	F&& adhocScript1, G&& adhocScript2, H&&... adhocScripts)
{
	auto status = Compare<TCompareStatus>(
		std::forward<F>(adhocScript1), std::forward<G>(adhocScript2),
		std::forward<H>(adhocScripts)...);
	status.m64Diff = M64Diff();
...
// Only to be used by ModifyCompare(). Will desync if used alone, as it assumes
// the caller will call Revert().
template <derived_from_specialization_of<Resource> TResource>
template <
	class TAdhocCustomScriptStatus,
	AdhocCustomStatusScript<TAdhocCustomScriptStatus> F>
AdhocScriptStatus<TAdhocCustomScriptStatus>
Script<TResource>::ExecuteAdhocNoRevert(F adhocScript)
{
	// Save state if performant
	OptionalSave();
