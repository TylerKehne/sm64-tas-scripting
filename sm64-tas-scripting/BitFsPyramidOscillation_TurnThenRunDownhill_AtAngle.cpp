...
bool BitFsPyramidOscillation_TurnThenRunDownhill_AtAngle::execution()
{
	...
	for (int i = 0; i < 50; i++)
	{
		...
		// too close to the lava Also make sure equillibrium point has been
		// passed
		if (
			status.maxSpeed >= _prevMaxSpeed &&
			status.framePassedEquilibriumPoint != -1)
