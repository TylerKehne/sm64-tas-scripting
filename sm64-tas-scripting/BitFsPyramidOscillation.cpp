...
bool BitFsPyramidOscillation::verification()
{
	...
	array<float, 3> normal = {
		floorObject->oTiltingPyramidNormalX, floorObject->oTiltingPyramidNormalY,
		floorObject->oTiltingPyramidNormalZ};
	AdvanceFrameRead();
	array<float, 3> normal2 = {
		floorObject->oTiltingPyramidNormalX, floorObject->oTiltingPyramidNormalY,
		floorObject->oTiltingPyramidNormalZ};
...
bool BitFsPyramidOscillation::execution()
{
	...
	auto stick					 = Inputs::GetClosestInputByYawExact(
							0x2000, 32, camera->yaw, initAngleStatus.downhillRotation);
	...
	for (int i = 0; i < 20; i++)
	{
		...
		auto turnRunStatus = Execute<BitFsPyramidOscillation_Iteration>(
			minFrame, maxFrame, CustomStatus.maxSpeed[i & 1],
			CustomStatus.initialXzSum, false);
			 ...
			 !turnRunStatus.validated))
		{
			...
			auto turnRunStatusBrake = Modify<BitFsPyramidOscillation_Iteration>(
				minFrameBrake, maxFrameBrake, CustomStatus.maxSpeed[(i & 1) ^ 1],
				CustomStatus.initialXzSum, true);
			if (turnRunStatusBrake.validated)
			{
				...
				auto turnRunStatus2 = Execute<BitFsPyramidOscillation_Iteration>(
					minFrame2, maxFrame2, CustomStatus.maxSpeed[i & 1],
					CustomStatus.initialXzSum, false);
				if (
					turnRunStatus2.passedEquilibriumSpeed >
