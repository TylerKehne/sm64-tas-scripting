...
bool BitFsPyramidOscillation_RunDownhill::execution()
{
	...
	for (int n = 0; n < 1000; n++)
	{
		...
		auto status = Test<GetMinimumDownhillWalkingAngle>(
			_roughTargetAngle, marioState->faceAngle[1]);
		...
		// Record initial tilt change direction and check if equilibrium point
		// has been passed
		int xTiltDirection =
		...
		if (
			...
			fabs(pyramid->oTiltingPyramidNormalX - prevNormalX) +
					fabs(pyramid->oTiltingPyramidNormalZ - prevNormalZ) >=
				0.0199999f)
		{
		...
		CustomStatus.finalXzSum = fabs(pyramid->oTiltingPyramidNormalX) +
