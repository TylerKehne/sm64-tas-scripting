// Restored from the tasfw-decomp library (commit 69792ad) for ROADMAP 4.1: the downhill-angle
// scripts call simulate_platform_tilt, which had lost its definition in the big refactor.
	...
	bool* isSlope)
{
	...
	Vec3f pyramidPos = {
		pyramidPlatform->oPosX, pyramidPlatform->oPosY, pyramidPlatform->oPosZ};
	Vec3f pyramidNormal = {
		pyramidPlatform->oTiltingPyramidNormalX,
		pyramidPlatform->oTiltingPyramidNormalY,
	...
	bhv_tilting_inverted_pyramid_loop(
		pyramidNormal, pyramidPos, marioPos, onPlatform, transform);
	...
	free(pyramidSurfaces); // get_surfaces mallocs them
	return found;
}

void bhv_tilting_inverted_pyramid_loop(
	Vec3f& platNormal, Vec3f& platPos, Vec3f& marioPos, bool onPlatform,
	Mat4& transform)
{
	float dx;
	float dy;
	...
	if (onPlatform)
	{
		...
		// conditionals since it needs to tilt regardless of whether Mario is
		// on.
